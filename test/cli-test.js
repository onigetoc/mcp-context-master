...
function parseArgs() {
  ...
  // Parse --tokens flag
  const tokensIndex = args.findIndex((arg) => arg.startsWith("--tokens="));
  if (tokensIndex !== -1) {
    result.tokens = parseInt(args[tokensIndex].split("=")[1]) || 1000;
  }
...
Usage:
  bun cli-test.js searchTool <query> [--topic <topic>] [--tokens=<number>]

Examples:
  bun cli-test.js searchTool "Remotion"
  bun cli-test.js searchTool "Remotion" --topic "caption from srt"
  bun cli-test.js searchTool "react components" --topic "ui library" --tokens=2000

Arguments:
  <query>           GitHub search query (required)
  --topic <topic>   Context7 topic filter (optional)
  --tokens=<num>    Token limit for Context7 (default: 1000)
