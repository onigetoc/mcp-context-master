...
All contributors must adhere to our \[Code of Conduct\](https://github.com/modelcontextprotocol/.github/blob/main/CODE\_OF\_CONDUCT.md).
...
The protocol layer handles message framing, request/response linking, and high-level communication patterns.
  ...
  \`\`\`python
    class Session(BaseSession\[RequestT, NotificationT, ResultT\]):
        ...
        async def \_received\_notification(
            self,
...
In this tutorial, you'll learn how to build a LLM-powered chatbot client that connects to MCP servers. It helps to have gone through the \[Server quickstart\](/quickstart/server) that guides you through the basic of building your first server.
  ...
  \[You can find the complete code for this tutorial here.\](https://github.com/modelcontextprotocol/kotlin-sdk/tree/main/samples/kotlin-mcp-client)
    ...
    Make sure your keep your \`ANTHROPIC\_API\_KEY\` secure!
    ...
    private val messageParamsBuilder: MessageCreateParams.Builder = MessageCreateParams.builder()
        .model(Model.CLAUDE\_3\_5\_SONNET\_20241022)
        .maxTokens(1024)
    ...
    sequenceDiagram
        ...
        Client<<->>MCP\_Server: Get available tools
        Client->>Claude: Send query with tool descriptions
        Claude-->>Client: Decide tool execution
        Client->>MCP\_Server: Request tool execution
        MCP\_Server->>Tools: Execute chosen tools
        Tools-->>MCP\_Server: Return results
        MCP\_Server-->>Client: Send results
  ...
  \[You can find the complete code for this tutorial here.\](https://github.io/modelcontextprotocol/csharp-sdk/tree/main/samples/QuickstartClient)
    ...
    dotnet user-secrets set "ANTHROPIC\_API\_KEY" ""
    \`\`\`
    ...
    \`\`\`csharp
    using IChatClient anthropicClient = new AnthropicClient(new APIAuthentication(builder.Configuration\["ANTHROPIC\_API\_KEY"\]))
        .Messages
        .AsBuilder()
    ...
    Here's an example of what it should look like it connected to a weather server quickstart:
      ...
      ![Image 5](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-dotnet-client.png)
...
We'll build a server that exposes two tools: \`get-alerts\` and \`get-forecast\`. Then we'll connect the server to an MCP host (in this case, Claude for Desktop):
  ...
  ![Image 6](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/weather-alerts.png)

  ![Image 7](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/current-weather.png)
...
This tutorial will primarily focus on tools.
    ...
    \`\`\`bash
      ...
      \`\`\`powershell
        code $env:AppData\\Claude\\claude\_desktop\_config.json
        \`\`\`
  ...
  This is a quickstart demo based on Spring AI MCP auto-configuration and boot starters.
    ...
    The \[Server Configuration Properties\](https://docs.spring.io/spring-ai/reference/api/mcp/mcp-server-boot-starter-docs.html#\_configuration\_properties) documents all available properties.
  ...
  Let's get started with building our weather server! \[You can find the complete code for what we'll be building here.\](https://github.com/modelcontextprotocol/kotlin-sdk/tree/main/samples/weather-stdio-server)
    ...
    \`\`\`kotlin build.gradle.kts
      plugins {
          kotlin("plugin.serialization") version "your\_version\_of\_kotlin"
          id("com.github.johnrengelman.shadow") version "8.1.1"
      }
      \`\`\`
      ...
      \`\`\`groovy build.gradle
      plugins {
          id 'org.jetbrains.kotlin.plugin.serialization' version 'your\_version\_of\_kotlin'
          id 'com.github.johnrengelman.shadow' version '8.1.1'
      }
      \`\`\`
...
Let's make sure Claude for Desktop is picking up the two tools we've exposed in our \`weather\` server. You can do this by looking for the hammer ![Image 8](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/claude-desktop-mcp-hammer-icon.svg) icon:

  ![Image 9](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/visual-indicator-mcp-tools.png)

After clicking on the hammer icon, you should see two tools listed:

  ![Image 10](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/available-mcp-tools.png)
...
\* What are the active weather alerts in Texas?
  ...
  ![Image 11](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/current-weather.png)

  ![Image 12](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/weather-alerts.png)
...
In this tutorial, you will extend \[Claude for Desktop\](https://claude.ai/download) so that it can read from your computer's file system, write new files, move files, and even search files.
  ...
  ![Image 13](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-filesystem.png)
...
This is what it should look like on a Mac:
  ...
  ![Image 14](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-menu.png)

Click on "Developer" in the left-hand bar of the Settings pane, and then click on "Edit Config":

  ![Image 15](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-developer.png)
...
Upon restarting, you should see a hammer ![Image 16](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/claude-desktop-mcp-hammer-icon.svg) icon in the bottom right corner of the input box:

  ![Image 17](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-hammer.png)

After clicking on the hammer icon, you should see the tools that come with the Filesystem MCP Server:

  ![Image 18](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-tools.png)
...
As needed, Claude will call the relevant tools and seek your approval before taking an action:
  ...
  ![Image 19](https://mintlify.s3.us-west-1.amazonaws.com/mcp/images/quickstart-approve.png)
...
Requires the `mcp-spring-webmvc` dependency.
      ...
      class McpConfig {
          ...
          @Bean
          RouterFunction mcpRouterFunction(WebMvcSseServerTransportProvider transportProvider) {
              return transportProvider.getRouterFunction();
          }
      }
      \`\`\`
...
**Aditional info from Google Gemini**

For an MCP (Model Context Protocol) server, a well-structured project is crucial for clarity, modularity, and scalability. A typical and recommended file/folder structure prioritizes separation of concerns, making the project easier to maintain and extend.
Recommended Project Structure:
Root Directory:
src/: Contains the core application logic.
controllers/ or routes/: Handles API endpoints and request routing.
services/: Implements business logic and interacts with models.
models/: Defines data models and potentially includes model weights or definitions.
utils/: Houses utility functions or helper modules.
config/: Stores environment-specific settings and configurations.
.env: For environment variables (e.g., database connection strings, API keys).
tests/: Contains unit and integration tests for various components.
docs/: Holds project documentation, user manuals, and API documentation.
assets/: If applicable, stores static assets like images, stylesheets, or other media.
Dockerfile: For containerization and deployment.
requirements.txt (Python) or package.json (Node.js): Lists project dependencies.
.gitignore: Specifies files and directories to be excluded from version control.
Best Practices for Project Structure:
Modularity:
Separate concerns into distinct directories (e.g., controllers, services, models) to promote reusability and maintainability.
Clarity:
Use meaningful and descriptive names for files and folders to enhance readability and understanding.
Consistency:
Maintain a consistent naming convention and structure throughout the project.
Configuration Management:
Use .env files for environment-specific settings to avoid hardcoding sensitive information.
Testing:
Dedicate a tests directory for comprehensive testing, promoting a test-driven development approach.
Documentation:
Maintain up-to-date documentation in a dedicated docs folder for easy reference.
Version Control:
Utilize .gitignore to manage version control effectively and exclude unnecessary files.



RESPONSE 2 GOOGLE SEARCH VIA GEMINI (show me mcp server best practices for project structure and tools used and and structure for nodejs project)

MCP Server Best Practices for Project Structure
A well-structured Model Context Protocol (MCP) server project, particularly for Node.js, emphasizes clarity, modularity, and scalability. A recommended structure includes:
Root Directory:
Contains package.json, .env for environment variables, and potentially README.md for documentation.
src Directory:
Houses the core application logic.
tools: Defines MCP tools with clear schemas, validation, and consistent JSON responses. 
services: Contains business logic and interactions with external systems (e.g., databases, APIs).
models: Defines data structures and potentially ORM/ODM models.
config: Manages application configurations.
utils: Stores helper functions, database connections, and common utilities.
index.js (or similar): The main entry point for the MCP server.
Tools Used in MCP Server Development
MCP SDK: Essential for defining and implementing MCP tools and the server itself (e.g., @modelcontextprotocol/sdk for Node.js).
Package Manager: npm or yarn for managing dependencies.
Linter: Tools like ESLint for code quality and consistency.
Testing Frameworks: Jest or Mocha for unit and integration testing of tools and services.
Environment Variable Management: dotenv for securely managing sensitive information like API keys.
Schema Validation: Zod or Joi for validating tool input and output schemas.
Node.js Project Structure for MCP Servers
A typical Node.js project structure for an MCP server would resemble:
Code
